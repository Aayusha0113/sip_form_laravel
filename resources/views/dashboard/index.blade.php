...
<div class="admin-main">
    ...
    <div class="table-section">
                    ...
                    <tr data-original-sip="{{ $originalSip }}">
                        ...
                        <td>
                            ...
                            <div class="action-cell">
                                ...
                                <a class="preview-btn" href="{{ route('dashboard.show', $row->sip_number) }}" title="View Details">
        ...
        <div id="rowActions" class="action-popover" role="dialog" aria-hidden="true">

            <button id="editBtn" class="action-edit" title="Edit this record"><i class="fas fa-edit"></i> Edit</button>

            <button id="deleteBtn" class="action-delete" title="Delete this record"><i class="fas fa-trash"></i> Delete</button>

            <button id="cancelSelectionBtn" class="action-cancel" title="Cancel"><i class="fas fa-times"></i> Cancel</button>
    ...
    <div id="confirmOverlay" class="modal-overlay" aria-hidden="true">
        ...
        <div class="modal" role="dialog" aria-modal="true">
            ...
            <h3 style="margin-top: 0; color: #d9534f;"><i class="fas fa-exclamation-triangle"></i> Confirm Delete</h3>

            <p id="confirmMessage">Are you sure you want to delete this record?</p>

            <div class="modal-actions">

                <button id="confirmDeleteBtn" class="modal-confirm"><i class="fas fa-trash"></i> Delete</button>

                <button id="cancelDeleteBtn" class="modal-cancel"><i class="fas fa-times"></i> Cancel</button>
...
<button class="floating-btn" onclick="window.location.href='{{ route('admin.dashboard') }}';" title="Go to Admin Dashboard">
...
<script>
    ...
    (function() {
        ...
        const popover = document.getElementById('rowActions');
        ...
        function updateTableDisplay() {
            ...
            rows.forEach(row => {
                ...
                if (companyFilter) {
                    ...
                    showRow = row.cells[3].textContent.toLowerCase().includes(companyFilter.toLowerCase());

                }



                // If company filter passed, apply search filter

                if (showRow && searchInput) {

                    let searchMatch = false;

                    const searchDN = parseInt(searchInput, 10);



                    if (searchOption === 'file') {

                        searchMatch = row.cells[0].textContent

                            .toLowerCase()

                            .includes(searchInput.toLowerCase());

                    } else if (searchOption === 'sip') {

                        searchMatch = row.cells[1].textContent

                            .toLowerCase()

                            .includes(searchInput.toLowerCase());

                    } else if (searchOption === 'dn') {

                        const dnRangeText = row.cells[2].textContent.trim();

                        if (dnRangeText.includes('-') && !isNaN(searchDN)) {

                            const [start, end] = dnRangeText.split('-').map(n => parseInt(n, 10));

                            searchMatch = searchDN >= start && searchDN <= end;

                        } else {

                            searchMatch = dnRangeText.toLowerCase().includes(searchInput.toLowerCase());

                        }

                    } else if (searchOption === 'company') {

                        searchMatch = row.cells[3].textContent
        ...
        function positionPopover(row) {
            ...
            const popoverHeight = 55; // Approximate height of popover



            // Position to the right of the row, centered vertically on the row

            let left = rowRect.right + 10;

            let top = rowRect.top + (rowRect.height / 2) - (popoverHeight / 2);
            ...
            if (left + popoverWidth > window.innerWidth) {
                ...
                left = rowRect.left - popoverWidth - 10;
        ...
        tableBody.addEventListener('click', (event) => {
            ...
            const previewLink = event.target.closest('.preview-btn');

            if (previewLink) return; // keep preview link behaviour
        ...
        const searchOptionEl = document.getElementById('searchOption');



        if (companyFilterEl) companyFilterEl.addEventListener('change', updateTableDisplay);
        ...
        editBtn.addEventListener('click', () => {
            ...
            window.location.href = "{{ route('dashboard.import.form') }}?sip=" + encodeURIComponent(sip);
        ...
        function showDeleteModal(sip) {
            ...
            pendingDeleteSip = sip;

            confirmMessage.textContent = `Are you sure you want to delete SIP ${sip}? This action cannot be undone.`;
        ...
        confirmDeleteBtn.addEventListener('click', () => {
            ...
            if (!pendingDeleteSip) return;



            const token = document.head.querySelector('meta[name="csrf-token"]').content;

            const deleteRoute = "{{ route('admin.companies.delete') }}";
        ...
        window.addEventListener('scroll', function() {
            ...
            if (st > lastScrollTop) {
                ...
                const tableBottom = tableWrapper.getBoundingClientRect().bottom;

                const viewportHeight = window.innerHeight;

                if (tableBottom < viewportHeight - 100) {
